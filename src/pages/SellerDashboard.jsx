...
const DEMO_STATS = {
  ...
  avg_rating: 4.8,
  recent_sales: [
    { note_title: "Digital Electronics Complete Notes", amount: 44 },
    { note_title: "Signals & Systems Short Notes", amount: 35 },
  ],
};
...
export default function SellerDashboard() {
  ...
  const [myNotes] = useState(DEMO_NOTES);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <Stat label="Total Revenue" value={`₹${stats.total_revenue}`} color="#4ADE80" icon={Wallet} />
        <Stat label="Wallet Balance" value={`₹${stats.wallet_balance}`} color="#F4FF47" icon={Wallet} />
        <Stat label="Notes Sold" value={stats.total_sold} color="#4C7BF4" icon={ChartBar} />
        <Stat label="Total Uploads" value={stats.total_uploads} color="#FF6B9E" icon={UploadSimple} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white border-2 border-black rounded-lg p-6 brutal-shadow">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex items-center justify-between">
              <span className="flex items-center gap-2"><Star size={16} /> Avg rating</span>
